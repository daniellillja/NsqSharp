        ...
        public void UpgradeTls(TlsConfig tlsConfig)
        {
            ...
            lock (_readLocker)
            {
                lock (_writeLocker)
                {
                    ...
                    try
                    {
                        sslStream.AuthenticateAsClientAsync(_hostname, new X509Certificate2Collection(), enabledSslProtocols, tlsConfig.CheckCertificateRevocation).RunSynchronously();
                    }
                    catch (Exception ex)
                    {
