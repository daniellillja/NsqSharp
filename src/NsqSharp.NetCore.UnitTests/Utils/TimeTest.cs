...
namespace NsqSharp.Tests.Utils
{
    ...
    public class TimeTest
    {
        private static readonly TestData<string, long> _parseDurationTests = new TestData<string, long>
        {
            ...
            {
                "1h2m3s4ms5us6ns", new Result<long>(1 * Time.Hour + 2 * Time.Minute + 3 * Time.Second +
                                                    4 * Time.Millisecond +
                                                    5 * Time.Microsecond + 6 * Time.Nanosecond)
            },
            {
                "39h9m14.425s",
                new Result<long>(39 * Time.Hour + 9 * Time.Minute + 14 * Time.Second + 425 * Time.Millisecond)
            },
            // large value
        ...
        public void TestParseDuration()
        {
            ...
            {
                var input = kvp.Key;
                var shouldPass = kvp.Value.ShouldPass;
                var expected = kvp.Value.Expected;
                var expectedException = kvp.Value.ExpectedException;

                if (shouldPass)
                {
                    var actual = Time.ParseDuration(input);
                    Assert.AreEqual(expected, actual, input);
                }
                else
                {
                    AssertHelper.Throws(expectedException, () => Time.ParseDuration(input));
                }
            }

            AssertHelper.Throws<ArgumentNullException>(() => Time.ParseDuration(null));
        }
        ...
        public void AfterFired()
        {
            ...
            {
                var allowRetry = tryNum < maxTries;
                try
                {
