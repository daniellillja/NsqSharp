...
namespace NsqSharp.Tests
{
    ...
    public class MockTest
    {
        ...
        public void TestConsumerRequeueNoBackoff()
        {
            ...
            var msgRequeueNoBackoff = new Message(msgIDRequeueNoBackoff,
                Encoding.UTF8.GetBytes("requeue_no_backoff_1"));
        ...
        public void TestConsumerBackoffDisconnect()
        {
            ...
            AssertHelper.AreEqual(expected, actual, "test1 failed");
            ...
            AssertHelper.AreEqual(expected, actual, "test2 failed");
            q.Stop();
        }
    }
            ...
            switch (body)
            {
                ...
                case "requeue_no_backoff_1":
                    if (message.Attempts > 1)
                        break;
                    message.RequeueWithoutBackoff(null);
                    break;
                case "good":
                    break;
                case "bad":
    ...
    public class mockNSQD
    {
        ...
        private instruction[] script { get; }
        private TcpListener tcpListener { get; }
        private IPAddress ipAddr { get; }
            ...
            using (var connw = new BinaryWriter(conn.GetStream()))
            {
                ...
                while (doLoop && idx < script.Length)
                    Select
                        ...
                        {
                            var strLine = Encoding.UTF8.GetString(line);
                            ...
                            switch (args[0])
                            {
                                case "IDENTIFY":
                                    try
                                    {
                                        var l = rdr.ReadBytes(4);
                                        var size = Binary.BigEndian.Int32(l);
                                        var b = rdr.ReadBytes(size);
