...
namespace NsqSharp.Api
{
    ...
    public abstract class NsqHttpApi
    {
        ...
        protected static byte[] Request(string endpoint, HttpMethod httpMethod, int timeoutMilliseconds, byte[] body = null)
        {
            ...
            webRequest.ContinueTimeout = timeoutMilliseconds;
            webRequest.Accept = "application/vnd.nsq; version=1.0";
            webRequest.Headers["User-Agent"] = string.Format("{0}/{1}", ClientInfo.ClientName, ClientInfo.Version);

            if (httpMethod == HttpMethod.Post && body != null && body.Length != 0)
            {
                webRequest.ContentType = "application/x-www-form-urlencoded";
                webRequest.Headers["Content-Length"] = body.Length.ToString();

                using (var request = webRequest.GetRequestStreamAsync().Result)
                {
                    request.Write(body, 0, body.Length);
                }
            }

            using (var httpResponse = (HttpWebResponse)webRequest.GetResponseAsync().Result)
